...
func GetElapsedTimeString(all_time int) string {
	...
	if all_time < 60 {
		elapsed_time = strconv.Itoa(all_time) + "秒"
	} else if all_time >= 60 && all_time < 60*60 {
		elapsed_second := strconv.Itoa(all_time%60) + "秒"
		elapsed_time = strconv.Itoa(all_time/60) + "分" + elapsed_second
	} else if all_time >= 3600 && all_time < 3600*24 {
		elapsed_h := strconv.Itoa(all_time/3600) + "时"
		m := all_time % 3600
		elapsed_m := strconv.Itoa(m/60) + "分"
		elapsed_s := strconv.Itoa(m%60) + "秒"
		elapsed_time = elapsed_h + elapsed_m + elapsed_s
	} else {
		elapsed_d := strconv.Itoa(all_time/(3600*24)) + "天"
		h := all_time % (3600 * 24)
		elapsed_h := strconv.Itoa(h/3600) + "时"
		m := h % 3600
		elapsed_m := strconv.Itoa(m/60) + "分"
		elapsed_s := strconv.Itoa(m%60) + "秒"
...
func GoDownload(wg *sync.WaitGroup, download_path, download_url string) {
	...
	if err != nil {
		log.Error("连接远程服务器时出错!" + err.Error())
	}
...
func GoInfo(wg *sync.WaitGroup, download_path, download_url string, start_time time.Time) {
	...
	for {
		...
		if ps < 100 {
			...
			showstr = "\r已下载: %s/%s [%s] 已完成:%s 当前下载速度:%s/s 已耗时:%s         "
			//因为还没有下载完成，所以下载速度就是上一秒和下一秒的差
			avg_str = sd_str
		} else {
			jdt = strings.Repeat("=", 50)
			download_percent = "100%"
			showstr = "\r已下载: %s/%s [%s] 已完成:%s 平均下载速度:%s/s 总耗时:%s          \n"
		}
		...
		//平均速度计算
		if size_int < 1024 {
			avg_str = strconv.Itoa(int(float32(size_int)/float32(all_time))) + "B/s"
		} else if 1024 <= size_int && size_int < 1024*1024 {
			avg_str = strconv.FormatFloat(float64(size_int)/1024/float64(all_time), 'f', 2, 64) + "K/s"
		} else {
			avg_str = strconv.FormatFloat(float64(size_int)/1024/1024/float64(all_time), 'f', 2, 64) + "M/s"
		}
...
//功能整合,将下载代码抽象到一个函数中
func UrlDownload(download_path, download_url string) (string, string) {
	//载入日志配置
	log := base.HttpSetLog()

	//清理下载目录
	dir_path := beego.AppConfig.String("download_path")
	err, out := base.ClearDir(dir_path)
	if err == "error" {
		log.Error("清理下载目录出现异常:" + out)
		return "error", "清理下载目录出现异常:" + out
	}

	//清理解压目录
	unzip_path := beego.AppConfig.String("unzip_path")
	err1, out1 := base.ClearDir(unzip_path)
	if err1 == "error" {
		log.Error("清理解压目录出现异常:" + out1)
		return "error", "清理解压目录出现异常:" + out1
	}
